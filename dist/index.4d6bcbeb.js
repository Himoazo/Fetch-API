...
async function fetchData() {
    ...
    try {
        ...
        for (let entry of experience){
            ...
            /* tr.contentEditable = true; */ tr.classList.add("exp");
            ...
            if (entry.enddate == "1899-11-29T23:00:00.000Z") td5.textContent = "P\xe5g\xe5ende";
            else td5.textContent = entry.enddate;
...
async function deleteEntry(id) {
    const confirm = window.confirm("\xc4r du s\xe4ker att du vill ta bort denna arbetserfarenhet?");
    if (confirm) try {
    ...
    else console.log("avbruten borttagning");
}
function editEntry(id) {
    ...
    if (expEdit.companyname && expEdit.jobtitle && expEdit.location && expEdit.startdate) putData(expEdit, id);
    else document.getElementById("editError").textContent = "V\xe4nligen fyll i alla obligatoriska f\xe4llt";
}
async function putData(expEdit, id) {
    ...
    try {
        ...
        if (!error) document.getElementById("editError").textContent = "En arbetserfarenhet har redigerats";
    }
}
...
addBtn.addEventListener("click", (event)=>{
    ...
    if (companyname && jobtitle && location && startdate) postData(companyname, jobtitle, location, startdate, enddate);
});
async function postData(companyname, jobtitle, location, startdate, enddate) {
    ...
    try {
        ...
        error = err;
        console.error("Fetch error:", error);
        document.getElementById("postErr").textContent = "Det gick inte att l\xe4gga till pga: " + error;
    } finally{
        if (!error) document.getElementById("printed").textContent = "En arbetserfarenhet har laggts till och kan synas p\xe5 startsidan";
    }
}

},{}]},["anvqh","gLLPy"], "gLLPy", "parcelRequire4176")
